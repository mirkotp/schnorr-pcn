...
class Node:
    ...
    def __init__(self, group, g, h, name, msg_sender, debug=False):
        self.group = group
        self.g = g
        self.h = h
        self.name = name
    ...
    def _msg_receive(self, msg):
        if msg["__expected_state__"] != self._state.__class__.__name__:
            raise Exception(
                f"({self.name}) Unexpected state {msg['__expected_state__']}: {self._state.__class__.__name__} expected."
            )
    ...
    def _nizk_verify(self, h, proof):
        u, c, z = proof
        return c == self.group.hash((self.g, h, u)) and self.g ** z == u * h**c

    def _pedersen_commit(self, m):
        r = self.group.random(ZR)
        c = (self.g**m) * (self.h**r)
        return (c, r)

    def _pedersen_verify(self, m, c, r):
        return c == (self.g**m) * (self.h**r)
...
class _WAIT_SETUP(_State):
    ...
    def msg_receive(self, msg) -> None:
        ...
        if not self.node._nizk_verify(Y, msg["proof"]):
...
class _LOCK_RECIPIENT_2(_State):
    def msg_receive(self, msg) -> None:
        ...
        com, decom = self.node._pedersen_commit(
            self.node.group.hash((R, *proof)))

        self.node._setState(
            _LOCK_RECIPIENT_4,
            {"r": r, "R": R, "amount": msg["amount"],
                "proof": proof, "decom": decom},
        )
        self.node._msg_send(
            self.node.leftNode, {"pk_next": self.node.pk,
                                 "com": com}, _LOCK_SENDER_3
        )
...
class _LOCK_SENDER_3(_State):
    def msg_receive(self, msg):
        ...
        self.node._setState(
            _LOCK_SENDER_5, {"r": r, "R": R, "com": msg["com"]})
        self.node._msg_send(
            self.node.rightNode,
            {"Rprev": R, "proof": self.node._nizk_prove(r)},
            _LOCK_RECIPIENT_4,
        )
...
class _LOCK_RECIPIENT_4(_State):
    def msg_receive(self, msg) -> None:
        ...
        if not self.node._nizk_verify(Rprev, msg["proof"]):
            self.node._abort_protocol("Invalid proof!")
        ...
        e = self.node.group.hash((self.node.pkL, rfactor, m))

        s = r + (e * self.node.sk)

        self.node._setState(
            _LOCK_RECIPIENT_6, {"rfactor": rfactor,
                                "R": R, "Rprev": Rprev, "e": e}
        )
        self.node._msg_send(
            self.node.leftNode,
...
class _LOCK_SENDER_5(_State):
    def msg_receive(self, msg) -> None:
        ...
        if not self.node._nizk_verify(Rnext, proof):
            self.node._abort_protocol("Invalid proof!")

        if not self.node._pedersen_verify(
            self.node.group.hash(
                (Rnext, *proof)), self.state_info["com"], msg["decom"]
        ):
        ...
        rfactor = R * Rnext * Y
        e = self.node.group.hash((self.node.pkR, rfactor, m))

        if (
            self.node.g**s
            != Rnext * (self.node.pkR / (self.node.g**self.node.sk)) ** e
        ):
        ...
        self.node._msg_send(
            self.node.rightNode, {"amount": amount,
                                  "m": m, "sp": sp}, _LOCK_RECIPIENT_6
        )
...
class _LOCK_RECIPIENT_6(_State):
    def msg_receive(self, msg) -> None:
        ...
        if self.node.k[1] == 0:
            self.node._setState(
                _LOCK_SENDER_1, {"amount": amount - self.node.transaction_fee}
            )
        else:
...
class _WAIT_RELEASE(_State):
    def msg_receive(self, msg) -> None:
        ...
        self.node._log(
            f"VALID KEY: {self.node._vf(self.node.LR, self.node.k)}")
